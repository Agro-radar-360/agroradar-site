...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const base = process.env.NEXT_PUBLIC_API_URL || 'https://agro-radar-360-3-0.onrender.com';
  ...
  ? json.articles.map((a: any) => ({
      id: a.id,
      title: a.title,
      summary: a.content ? a.content.substring(0, 150) + (a.content.length > 150 ? '...' : '') : 'Sem resumo',
      category: a.category || 'Agronegócio',
      image: a.image,
      date: a.published_at ? new Date(a.published_at).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: '2-digit' }) : '',
      author: 'AGRO-RADAR',
      readTime: '3 min'
    }))
  : [];
